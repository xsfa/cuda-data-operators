"""
Low-level Python bindings for libdataops.so CUDA kernels.

The shared library must be compiled before use:
...
class DataOps:
    ...
    def __init__(self, lib_path: str | None = None):
        if lib_path is None:
            # Package sits one level below the repo root where the .so lives
            path = Path(__file__).parent.parent / "libdataops.so"
        else:
            path = Path(lib_path)
        path = path.resolve()
        ...
        if not path.exists():
            raise FileNotFoundError(
                f"{path} not found. Compile with: python test_runner.py --setup"
            )
    ...
    def prefix_scan(
        self, input_ptr: int, output_ptr: int, n: int, temp_ptr: int
    ) -> None:
    ...
    def filter_int32(
        ...
        """Filter int32 column. Returns number of matching rows."""
    ...
    def pool_destroy(self, pool: int) -> None:
        self._lib.pool_destroy(pool)

    def pool_reset(self, pool: int) -> None:
        self._lib.pool_reset(pool)

    def pool_used(self, pool: int) -> int:
        return self._lib.pool_used(pool)

    def pool_capacity(self, pool: int) -> int:
        return self._lib.pool_capacity(pool)
